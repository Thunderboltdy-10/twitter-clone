            ...
            <div className={(compact ? "items-center " : "") + "flex"}>
                ...
                <div className="grow pl-2">
                    <Upload onUploadFinish={src => setImages(prev => [...prev, src])}>
                        {({isUploading}) => (
                            <div>
                                <textarea className={(compact ? "h-10 mt-1" : "h-24") + " w-full p-2 bg-transparent text-twitterWhite"} value={text} onChange={e => setText(e.target.value)} placeholder={placeholder}/>
                                <div className="flex -mx-2">
                                    {images.length > 0 && images.map(img => (
                                        <div key={img} className="h-24 m-2">
